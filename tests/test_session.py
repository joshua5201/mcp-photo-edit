...
class DummyBackend:
    backend_id: str = "dummy-backend"
    state_file_name: str = "session.state"
    ...
    def __init__(self) -> None:
        ...
        self.preview_size: tuple[int, int] = (800, 600)
    ...
    def render_export(
        self, source_path: Path, state_path: Path, target_path: Path
    ) -> tuple[int, int]:
    ...
    def diagnostics_for(self, output_path: Path) -> DiagnosticSummary | None:
        return DiagnosticSummary.model_validate(
            ImageDiagnostics().analyze(output_path).model_dump()
        )
...
def test_create_session_persists_initial_history_state_and_preview(tmp_path: Path) -> None:
    ...
    history_state = session_dir / "history" / "step-0001.state"
    ...
    assert session.history[0].state_path is not None
    assert Path(session.history[0].state_path).name == "step-0001.state"
...
def test_apply_and_reset_adjustments_append_semantic_history(tmp_path: Path) -> None:
    ...
    assert updated.state_path is not None
    ...
    assert updated.diagnostic_dashboard_path is None
    assert updated.history[-1].diagnostic_dashboard_path is None
...
def test_crop_preview_does_not_overwrite_canonical_source_dimensions(tmp_path: Path) -> None:
    ...
    updated = manager.apply_adjustments(
        session.session_id,
        AdjustmentPatch(crop=CropAdjustment(left=0.1, top=0.1, right=0.9, bottom=0.9)),
        render_preview=True,
    )
...
def test_advanced_image_info_fail_open_preserves_preview(
    ...
    def fail_diagnostics(_output_path: Path) -> DiagnosticSummary | None:
        raise RuntimeError("diagnostics failed")

    monkeypatch.setattr(backend, "diagnostics_for", fail_diagnostics)
...
def test_undo_and_redo_move_cursor_without_creating_new_steps(tmp_path: Path) -> None:
    ...
    session = manager.create_session(str(source))
    session = manager.apply_adjustments(
        session.session_id, AdjustmentPatch(exposure=1.0), render_preview=True
    )
    session = manager.apply_adjustments(
        session.session_id, AdjustmentPatch(contrast=20.0), render_preview=True
    )
...
def test_list_supported_adjustments_includes_new_rawtherapee_controls(tmp_path: Path) -> None:
    ...
    mixer_example = supported["rgb_mixer"].example
    assert isinstance(mixer_example, dict)
    assert mixer_example["green"] == [0.0, 95.0, 5.0]
...
def test_export_uses_cursor_selected_state(tmp_path: Path) -> None:
    ...
    session = manager.create_session(str(source))
    session = manager.apply_adjustments(
        session.session_id, AdjustmentPatch(exposure=1.0), render_preview=False
    )
    session = manager.apply_adjustments(
        session.session_id, AdjustmentPatch(contrast=20.0), render_preview=False
    )
